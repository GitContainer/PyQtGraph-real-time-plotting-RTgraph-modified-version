...
# Loads everything about mainWindowPPG configuration        
    ...
    def _update_plot(self):
        ...
        zeroDesR = 0.5413   # Desplacement from zero for Red signal
        ampIR = 1.0814       # amplitud for Red signal
        zeroDesIR = 2.5413   # Desplacement from zero for Red signal
        # generate the time
        tsignal = time() - self.timestamp
        # Spo2 Red signal function
        sR= zeroDesR + ampR * (0.05*np.sin(2*np.pi*tsignal*3*f)
        + 0.4*np.sin(2*np.pi*tsignal*f) + 0.25*np.sin(2*np.pi*tsignal*2*f+1))
        # Spo2 InfraRed signal function
        sIR= zeroDesIR + ampIR * (0.05*np.sin(2*np.pi*tsignal*3*f)
        + 0.4*np.sin(2*np.pi*tsignal*f) + 0.25*np.sin(2*np.pi*tsignal*2*f+1))
